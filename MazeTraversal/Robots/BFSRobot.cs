...
class BreadthFirstSearchRobot : BaseMappingRobot {
    private Queue<Tuple<int, int>> queue = new Queue<Tuple<int, int>>();
    private HashSet<Tuple<int, int>> visited = new HashSet<Tuple<int, int>>();

    public BreadthFirstSearchRobot(Func<bool> readSensorLeft, Func<bool> readSensorRight, Func<bool> readSensorForward, Func<bool> readSensorUp, Func<Action, bool> executeAnAction)
